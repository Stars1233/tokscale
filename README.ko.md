...
네이티브 Rust 모듈은 병렬 파일 스캐닝과 SIMD JSON 파싱을 통해 처리 속도를 크게 끌어올립니다(약 10배):
...
네이티브 모듈이 사용 가능할 때는 무거운 연산을 Rust에서 처리합니다. 네이티브 모듈이 설치되지 않은 경우에는, 호환성을 위해 TypeScript 구현으로 자동 폴백됩니다(다만 성능은 느릴 수 있습니다).
