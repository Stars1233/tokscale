...
impl<T: Clone + 'static> DialogContent for DialogSelect<T> {
    fn desired_size(&self, viewport: Rect) -> (u16, u16) {
        let width = 60u16.min(viewport.width.saturating_sub(4));
        let height = 20u16.min(viewport.height.saturating_sub(4));
        (width, height)
    }
    ...
    fn render(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let filter_text = if self.filter.is_empty() {
            Span::styled(&self.placeholder, Style::default().fg(theme.muted))
        } else {
